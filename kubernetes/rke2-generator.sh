...
FW_NAME=$(
/usr/sbin/hex_cli -c firmware list | awk '
/\[ACTIVE\]/{
    sub(/^[0-9]+:[ ]*/,"")
    sub(/[ ]*\[ACTIVE\].*/,"")
    print
...
FW_DATE=$(echo "$FW_NAME" | awk -F'_' '{print $3}' | cut -d'-' -f1)

SECRET_SEED=$(awk -F': *' '/secret-seed/ {print $2}' /etc/policies/cubesys/cubesys1_0.yml)

if [[ "$FW_DATE" -ge 20260301 ]]; then
